...
type Hour = i32;
type Minute = i32;
const TOTAL_HOURS_DAY: Hour = 24;
const TOTAL_MINUTES: Minute = 60;
...
impl Clock {
    pub fn new(hours: i32, minutes: i32) -> Self {
        let min_mod = minutes % TOTAL_MINUTES;
        let min_quo = minutes / TOTAL_MINUTES;
        let hour_add_by_min_quo = if min_mod < 0 { min_quo - 1 } else { min_quo };
        let hour_mod = (hours + hour_add_by_min_quo) % TOTAL_HOURS_DAY;

        Self {
            hours: if hour_mod >= 0 {
                hour_mod
            } else {
                TOTAL_HOURS_DAY + hour_mod
            },

            minutes: if min_mod >= 0 {
                min_mod
            } else {
                TOTAL_MINUTES + min_mod
            },
        }
    }

    pub fn add_minutes(&self, minutes: i32) -> Self {
        let min_mod = (self.minutes + minutes) % TOTAL_MINUTES;
        let min_quo = (self.minutes + minutes) / TOTAL_MINUTES;
        let hour_mod = if min_mod < 0 {
            (self.hours + min_quo - 1) % TOTAL_HOURS_DAY
        } else {
            (self.hours + min_quo) % TOTAL_HOURS_DAY
        };
